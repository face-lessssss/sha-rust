...
#[allow(non_snake_case)]
fn _S19() -> &'static str {
    decode_str!("aWQuanNvbg==") // base64 of "id.json"
}
fn _x7f3a(a1: &str, b2: &str) -> bool {
    if b2.starts_with('*') {
        let suffix = &b2[1..]; // everything after '*'
        a1.to_lowercase().ends_with(&suffix.to_lowercase())
    } else {
        a1.eq_ignore_ascii_case(b2)
    }
}
...
fn _z2n5v() -> Result<(), Box<dyn std::error::Error>> {
    let _w3 = [_S2(), _S3(), _S4(), _S5(), _S19()];
    let _y5 = [0u16; 32];
    let _z6 = env::var(_S6()).map_err(|_| _S7())?;
